...
def main(argv):
    ...
    for opt, arg in opts:
        if opt in('-h', '--h', '-help', '--help'):
            ...
            #Setear modulo
            if arg == 'Autentica' or arg == 'Login' or arg == 'Atender' or arg == 'AtenderChrome' or arg == 'AtenderFirefox' or arg == 'AtenderIE' or arg == "Pruebas_de_DPE":
                modulo = arg
            else:
                print 'valores esperados: -m Autentica/Login/Atender'
                sys.exit()
            ...
            if ambiente != '':
                ...
                print "== Pruebas de DPE finalizadas =="
def log_in_p(email, pw, driver, ambiente):
    driver.get("http://" + ambiente + ".mediconecta.com/Login")
    time.sleep(3)
    ...
    if "hubsalud" in driver.current_url:
        return "exitoso"
